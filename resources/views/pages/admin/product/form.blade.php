...
@section('title')
    <title>Otaku Store | Product Edit Page</title>
@endsection
    ...
    <div class="container py-5">
        ...
        <div class="card">
            ...
            <div class="card-header">
                <h3 class="mb-0">Form Edit Product</h3>
                <p class="text-sm mb-0 mt-2">
                    Pada halaman ini Admin dapat mengubah data product, yuk
                    kita manage data produk bersama-sama 😊. Pastikan semua field terisi yah.
                </p>
            </div>
            <!-- Card body -->
            <div class="card-body">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control  @error('name') is-invalid @enderror"
                                    id="name" placeholder="Nama Produk" name="name" value="{{ $products->name }}">
                            </div>
                            @if ($errors->has('name'))
                                ...
                                <select class="form-control @error('category_id') is-invalid @enderror" id="category_id"
                                    ...
                                    @foreach ($categories as $item)
                                        <option value="{{ $item->id }}"
                                            {{ $products->category_id == $item->id ? 'selected' : '' }}>{{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input class="form-control @error('release_date') is-invalid @enderror" type="date"
                                    id="release_date" name="release_date" value="{{ $products->release_date }}">
                            </div>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('status') is-invalid @enderror" id="status"
                                    ...
                                    <option value="Ready" {{ $products->status == 'Ready' ? 'selected' : '' }}>
                                        Ready
                                    </option>
                                    <option value="Pre-Order" {{ $products->status == 'Pre-Order' ? 'selected' : '' }}>
                                        Pre-Order
                                    </option>
                                    <option value="Run Out" {{ $products->status == 'Run Out' ? 'selected' : '' }}>
                                        Run Out
                                    </option>
                                </select>
                            </div>
